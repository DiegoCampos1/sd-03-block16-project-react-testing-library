...
describe('PokemonDetails.js component tests', () => {
  ...
  test('renders location section with correct info', () => {
    ...
    expect(locationHeading.textContent).toBe(`Game Locations of ${mockedPokemons[0].name}`);
    ...
    expect(locationMaps[0]).toHaveAttribute('src', `${mockedPokemons[0].foundAt[0].map}`);
    expect(locationMaps[1]).toHaveAttribute('src', `${mockedPokemons[0].foundAt[1].map}`);
  });
