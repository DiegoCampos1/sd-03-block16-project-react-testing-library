...
const { averageWeight, name, image, id, type } = testPokemon;
...
describe('Pokemon.js component tests', () => {
  ...
  test('renders one card with favorite pokemon info, and details link', () => {
    ...
    expect(pokemonAverageWeight.textContent).toBe(`Average weight:${value}${measurementUnit}`);
  ...
  test('test pokemon details link functionality', () => {
    const { getByText, history } = renderWithRouter(<Pokemon pokemon={testPokemon} isFavorite />);
